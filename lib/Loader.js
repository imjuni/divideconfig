...
var collectFormat = ['.json', '.hjson', '.yaml', '.yml'];
var collectConfig = [];

collectFormat.forEach(function (format) {
  collectConfig.push('config' + format);
});

function isCollectConfig (filename) {
  var isCollect = false;

  collectConfig.forEach(function (config) {
    isCollect = isCollect || filename.indexOf(config) >= 0;
  });
  ...
  while (queue.length > 0) {
    ...
    for (i = 0, len = elements.length; i < len; i++) {
      ...
      if (stat.isDirectory()) {
        ...
        if (collectFormat.indexOf(ext) >= 0 && isCollectConfig(elements[i])) {
          files.push({
            name: elements[i],
            path: currentDir,
            format: ext
...
function readWithStrip (pathWithName, format, readFunc) {
  try {
    ...
    if (format === '.yaml' || format === '.yml') {
      return yaml.safeLoad(readFunc(pathWithName).toString());
